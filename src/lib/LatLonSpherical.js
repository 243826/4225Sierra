...
class LatLonSpherical {
  ...
  set lat(lat) {
    this._lat = isNaN(lat)
      ? Dms.wrap90(Dms.parse(lat))
      : Dms.wrap90(Number(lat));
  ...
  set lon(lon) {
    this._lon = isNaN(lon)
      ? Dms.wrap180(Dms.parse(lon))
      : Dms.wrap180(Number(lon));
  ...
  static intersection(p1, brng1, p2, brng2) {
    ...
    if (Math.abs(δ12) < Number.EPSILON)
      return new LatLonSpherical(p1.lat, p1.lon); // coincident points
